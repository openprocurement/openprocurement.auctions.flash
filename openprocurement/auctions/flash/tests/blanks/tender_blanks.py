...
def create_role(self):
    ...
    self.assertEqual(set(Auction._fields) -
                     Auction._options.roles['create'].fields, fields)
...
def create_auction_invalid(self):
    ...
    response = self.app.post_json(
        request_path, {
            'data': {
                'enquiryPeriod': {
                    'endDate': 'invalid_value'}}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(
    ...
    response = self.app.post_json(
        request_path, {
            'data': {
                'enquiryPeriod': {
                    'endDate': '9999-12-31T23:59:59.999999'}}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    ...
    self.assertEqual(response.json['errors'], [{u'description': [
                     u'period should begin after enquiryPeriod'], u'location': u'body', u'name': u'tenderPeriod'}])

    now = get_now()
    test_auction_data['awardPeriod'] = {
    ...
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [{u'description': [
                     u'period should begin after tenderPeriod'], u'location': u'body', u'name': u'awardPeriod'}])

    test_auction_data['auctionPeriod'] = {
        'startDate': (
            now +
            timedelta(
                days=15)).isoformat(),
    ...
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [{u'description': [
                     u'CAV group of items be identical'], u'location': u'body', u'name': u'items'}])
...
def create_auction(self):
    ...
    self.assertEqual(response.content_type, 'application/json')
    auction = response.json['data']
    self.assertEqual(set(auction) - set(test_auction_data),
                     set([u'id',
                          u'dateModified',
                          u'auctionID',
                          u'date',
    ...
    response = self.app.post_json(
        '/auctions?opt_jsonp=callback', {"data": test_auction_data})
    self.assertEqual(response.status, '201 Created')
    ...
    response = self.app.post_json(
        '/auctions', {"data": test_auction_data, "options": {"pretty": True}})
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
    self.assertIn('{\n    "', response.body)
...
def patch_auction(self):
    ...
    response = self.app.patch_json(
        '/auctions/{}?acc_token={}'.format(
            ...
            "data": {
                "guarantee": {
                    "amount": 12, "valueAddedTaxIncluded": True}}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(
        response.json['errors'][0], {
            u'description': {
                u'valueAddedTaxIncluded': u'Rogue field'}, u'location': u'body', u'name': u'guarantee'})
    ...
    self.assertEqual(
        ...
        "Can't update auction in current (complete) status")
...
def first_bid_auction(self):
    ...
    self.app.patch_json(
        '/auctions/{}/awards/{}/complaints/{}?acc_token={}'.format(
            auction_id, award_id, complaint_id, owner_token), {
            "data": {
                "status": "answered", "resolutionType": "resolved", "resolution": "resolution text " * 2}})
    # satisfying resolution
    self.app.patch_json(
        '/auctions/{}/awards/{}/complaints/{}?acc_token={}'.format(
            auction_id, award_id, complaint_id, complaint_owner_token), {
            "data": {
