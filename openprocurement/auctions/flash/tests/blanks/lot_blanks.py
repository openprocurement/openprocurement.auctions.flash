...
def patch_auction_currency(self):
    ...
    response = self.app.post_json(
        '/auctions/{}/lots'.format(self.auction_id), {'data': self.test_lots[0]})
    ...
    response = self.app.patch_json(
        '/auctions/{}'.format(self.auction_id), {"data": {"value": {"currency": "GBP"}}}, status=422)
    ...
    response = self.app.patch_json(
        '/auctions/{}'.format(
            ...
            "data": {
                "value": {
                    "currency": "GBP"}, "minimalStep": {
                        "currency": "GBP"}}})
    self.assertEqual(response.status, '200 OK')
    # log currency is updated too
    response = self.app.get(
        '/auctions/{}/lots/{}'.format(self.auction_id, lot['id']))
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    lot = response.json['data']
    ...
    response = self.app.patch_json(
        '/auctions/{}/lots/{}'.format(
            self.auction_id, lot['id']), {
            "data": {
                "value": {
                    "currency": "USD"}}})
    self.assertEqual(response.status, '200 OK')
    ...
    # try to update lot minimalStep currency and lot value currency in single
    # request
    response = self.app.patch_json(
        '/auctions/{}/lots/{}'.format(
            self.auction_id, lot['id']), {
